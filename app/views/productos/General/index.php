    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <div class="flex items-center">
                ...
                <h1 class="text-xl font-semibold text-gray-800">Administrar productos</h1>
            </div>
                    ...
                    <?php if (!empty($productos)): ?>
                        <?php foreach ($productos as $p): ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-3"><?= htmlspecialchars($p['producto']) ?></td>
                                <td class="px-6 py-3"><?= htmlspecialchars($p['codigo']) ?></td>
                                <td class="px-6 py-3">S/ <?= number_format((float)$p['precio'], 2) ?></td>
                                <td class="px-6 py-3"><?= htmlspecialchars($p['categoria']) ?></td>
                                <td class="px-6 py-3"><?= htmlspecialchars($p['negocio']) ?></td>
                                <td class="px-6 py-3">
                                    <span class="px-2 py-1 text-xs rounded
                                        <?= ($p['activo'] ?? 0) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' ?>">
                                        <?= ($p['activo'] ?? 0) ? 'Activo' : 'Inactivo' ?>
                                    </span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    ...
                    <?php if (!empty($productos)): ?>
                        <?php foreach ($productos as $prod): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-3">
                                    <span class="px-2 py-1 text-xs rounded
                                        <?= ($prod['activo'] ?? 0) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' ?>">
                                        <?= ($prod['activo'] ?? 0) ? 'Activo' : 'Inactivo' ?>
                                    </span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
